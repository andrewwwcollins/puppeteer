...
class NetworkManager extends EventEmitter {
  ...
  _handleRequestRedirect(request, redirectStatus, redirectHeaders) {
    const response = new Response(this._client, request, request._response, redirectStatus, redirectHeaders);
  ...
  _onResponseReceived(event) {
    ...
    if (!request)
      return;
    const response = new Response(this._client, request, event.response, event.response.status, event.response.headers);
    request._response = response;
    this.emit(NetworkManager.Events.Response, response);
  }
...
class Response {
   ...
   */
  constructor(client, request, response = null, status, headers) {
    this._client = client;
